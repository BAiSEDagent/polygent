...
class LiveDataService extends EventEmitter {
  ...
  private async refreshMarkets(): Promise<void> {
    try {
      ...
      const now = Date.now();
      const tradable = markets.filter(m => {
        const endMs = m.endDate ? new Date(m.endDate).getTime() : NaN;
        return m.active && !m.closed && !!m.conditionId && m.tokenIds.length > 0 && Number.isFinite(endMs) && endMs > now;
      });
      ...
      const liveMarkets: LiveMarket[] = tradable.map(m => {
        ...
        priceHistory.push({ price: yesPrice, timestamp: nowTs });
        if (priceHistory.length > this.MAX_PRICE_HISTORY) {
          priceHistory.shift();
        }

        // Calculate 24h change
        const oneDayAgo = nowTs - 86_400_000;
      ...
      for (const lm of liveMarkets) {
        this.marketCache.set(lm.id, {
          data: lm,
          expiresAt: nowTs + this.MARKET_TTL * 2, // Keep longer than poll interval
        });
      }
