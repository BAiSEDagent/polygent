...
async function bootstrap(): Promise<void> {
  ...
  // 0. Initialize market metadata cache
  const { initMarketMetadataTable } = await import('./services/market-metadata');
  initMarketMetadataTable();

  // 0.5. Backfill builder fees from historical trades (run once on first deployment)
